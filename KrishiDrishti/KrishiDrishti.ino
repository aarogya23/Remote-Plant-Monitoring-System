// ================================================================
//  Krishi Drishti — Remote Plant Monitoring System
...
//    Sub-page B: Light Status + Light Advice
//
...
const char* SERVER_URL    = "http://192.168.137.207:8080/api/data";
...
// ================================================================
//  BH1750 FIX: In CONTINUOUS_HIGH_RES_MODE the sensor is always
//  measuring. Instead of gating on measurementReady() (which can
//  return false for the first ~200ms and leave g_lux at 0 = "Too Dark"),
//  we call readLightLevel() unconditionally and only skip the update
//  if the returned value is negative (sensor error).
// ================================================================
void readLight() {
  ...
  // If lux < 0 it's a sensor error — keep previous g_lux value
...
// ================================================================
//  LCD2 UPDATE — Soil/Env ↔ Light Detail (flips every 4s)
//
//  Sub-page 0:
//    Line 1: "Soil:65%  Moist "
//    Line 2: "T:28.4C  H:65%  "
//
//  Sub-page 1:  ← CHANGED: now shows light status + advice
//    Line 1: "Light:Good Light"
//    Line 2: "Plant is happy:)"
// ================================================================
void updateLCD2() {
  lcd2_clear();

  if (lcd2Page == 0) {
    // --- Sub-page 0: Soil + Temp + Humidity ---
    lcd2_setCursor(0, 0);
    lcd2_print("Soil:");
    ...
    // --- Sub-page 1: Light Status + Advice ---
    lcd2_setCursor(0, 0);
    lcd2_print("Light:");
    lcd2_print(g_lightStatus);

    // Pad/truncate advice to fill the 16-char line cleanly
    String advice = g_lightAdvice;
    while (advice.length() < 16) advice += " ";
    lcd2_setCursor(0, 1);
    lcd2_print(advice.substring(0, 16));
  }
}
...
void setup() {
  ...
  delay(500); // Let power supply settle before LCD init
  ...
  if (lightMeter.begin(BH1750::CONTINUOUS_HIGH_RES_MODE, 0x23, &Wire)) {
    ...
    delay(180); // BH1750 needs ~120-180ms for first measurement in high-res mode
  } else {
  ...
  Serial.println("LCD2 → GPIO25/26  |  Soil + Temp + Light Detail");
